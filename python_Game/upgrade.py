...
class Upgrade:
    ...
    def input(self):
        ...
        if self.can_move:
            ...
            if keys[pygame.K_RIGHT] and self.selection_id < self.attribute_number - 1:
                self.selection_id += 1
                self.can_move = False
                self.selection_time = pygame.time.get_ticks()
            elif keys[pygame.K_LEFT] and self.selection_id >= 1:
                self.selection_id -= 1
                self.can_move = False
                self.selection_time = pygame.time.get_ticks()
                ...
                self.can_move = False
                self.selection_time = pygame.time.get_ticks()
                self.item_list[self.selection_id].trigger(self.player)
    ...
    def create_items(self):
        ...
        for item, index in enumerate(range(self.attribute_number)):
            # horizontal position
            full_width = top = self.display_surface.get_size()[0]
    ...
    def display(self):
        ...
        for index, item in enumerate(self.item_list):
            ...
            item.display(self.display_surface, self.selection_id, name, value, max_value, cost)
